...
class enrol_boleto_plugin extends enrol_plugin {
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($instance->enrol !== 'boleto') {
             throw new coding_exception('Invalid enrol instance type!');
        }

        $context = context_course::instance($instance->courseid);
        if (has_capability('enrol/boleto:config', $context)) {
            $managelink = new moodle_url('/enrol/boleto/edit.php', array('courseid'=>$instance->courseid, 'id'=>$instance->id));
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if (has_capability('enrol/boleto:config', $context)) {
            $editlink = new moodle_url("/enrol/boleto/edit.php", array('courseid'=>$instance->courseid, 'id'=>$instance->id));
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/boleto:config', $context)) {
            return NULL;
        }
        // Multiple instances supported - different roles with different password.
        return new moodle_url('/enrol/boleto/edit.php', array('courseid'=>$courseid));
    }
     ...
     */
    public function enrol_boleto(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        $enrolstatus = $this->can_boleto_enrol($instance);

        // Don't show enrolment instance form, if user can't enrol using it.
        if (true === $enrolstatus) {
            $form = new enrol_boleto_enrol_form(NULL, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_boleto($instance, $data);
                }
            }
    ...
    /**
     * Checks if user can boleto enrol.
     *
     * @param stdClass $instance enrolment instance
     ...
     */
    public function can_boleto_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more boleto enrolments here.
                return get_string('maxenrolledreached', 'enrol_boleto');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_boleto', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_boleto_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_boleto');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_boleto_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    public function get_instance_defaults() {
        ...
        $fields['customint5']      = 0;
        $fields['customint6']      = $this->get_config('boletos');
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_boleto', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_boleto', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying boleto-enrolments...');
        ...
        // Note: the logic of boleto enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'boleto' AND e.customint2 > 0)
        ...
        $trace->output('...user boleto-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for boleto enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/boleto:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/boleto:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/boleto:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/boleto:manage", $context)) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent boleto enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or boleto enrol.
