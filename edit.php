...
require_capability('enrol/boleto:config', $context);

$PAGE->set_url('/enrol/boleto/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
/** @var enrol_boleto_plugin $plugin */
$plugin = enrol_get_plugin('boleto');

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'boleto', 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_boleto_edit_form(NULL, array($instance, $plugin, $context));
...
if ($mform->is_cancelled()) {
    ...
    if (!isset($data->customint6)) {
        // Add previous value of boletos if disabled.
...
$PAGE->set_title(get_string('pluginname', 'enrol_boleto'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_boleto'));
$mform->display();
echo $OUTPUT->footer();
