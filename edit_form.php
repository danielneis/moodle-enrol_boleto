...
/**
 * Adds new instance of enrol_boleto to specified course
...
class enrol_boleto_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_boleto'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_boleto');

        $options = array(1 => get_string('yes'), 0 => get_string('no'));
        $mform->addElement('select', 'customint6', get_string('boletos', 'enrol_boleto'), $options);
        $mform->addHelpButton('customint6', 'boletos', 'enrol_boleto');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_boleto'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_boleto');
        ...
        $options = array(1 => get_string('yes'),
                         0 => get_string('no'));
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_boleto'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_boleto');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_boleto'), $roles);

        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_boleto'), array('optional' => true, 'defaultunit' => 86400));
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_boleto');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_boleto'), array('optional' => true));
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_boleto');

        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_boleto'), array('optional' => true));
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_boleto');
        ...
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_boleto'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_boleto');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_boleto'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_boleto');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_boleto'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_boleto');
        } else {
        ...
        $mform->addElement('advcheckbox', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_boleto'));
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_boleto');

        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_boleto'), array('cols'=>'60', 'rows'=>'8'));
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_boleto');
        ...
        if (enrol_accessing_via_instance($instance)) {
            $mform->addElement('static', 'boletowarn', get_string('instanceeditboletowarning', 'core_enrol'), get_string('instanceeditboletowarningtext', 'core_enrol'));
        }
    ...
    function validation($data, $files) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_boleto');
            }
        }
    ...
    /**
    * Gets a list of roles that this user can assign for the course as the default for boleto-enrolment.
    *
    * @param context $context the context.
    * @param integer $defaultrole the id of the role that is set as the default for boleto-enrolment
